...
*/
 ...
 // if ((strlen($_POST['gengo']) == 0) && (!preg_match("/^[0-9]+$/",$_POST['year']))) {
  if (!preg_match("/^[0-9]+$/",$_POST['year'])){
     $errmsg .= "卒業年が入力されていません。<BR>";
  }

  if ($errmsg != "") {
    //いずれかの入力エラーがあったときは戻るボタン表示
    print $errmsg . "<BR>前の画面に戻って入力内容を確認してください。
     <FORM  name='check' action='user_upd.php' method='POST' enctype='multipart/form-data'>
...
function edit()
{
  document.regform.action="user_upd.php";
  ...
  <FORM name='regform' action='user_upd_exe.php' method='POST'>
...
*/
 print dfirst($con,"name","college","college_id=".$_POST['college_id'])
    ."&nbsp;".dfirst($con,"name","college_dept","college_id=".$_POST['college_id']." and dept_id=".$_POST['dept_id']);
...
<?php
 foreach($_POST as $key=>$val){
   print "<input type='hidden' name='$key' value='$val'> <INPUT type='hidden' name='proc' value='update'>";
 }
?>
