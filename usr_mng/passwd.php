...
/****************************************/
  ...
  if((strlen($_POST['password1'])>0)&&(strlen($_POST['password2'])>0)) {
    //現在のパスワードが入力されたとき
    if(strlen($_SESSION['almuni_org'])>0){
	  //DBからパスワード抽出し比較
      $sql="SELECT password FROM college WHERE login_id =".$_SESSION['login_id'];
      $rst=mysql_query($sql,$con);
      $col=mysql_fetch_array($rst);
      if($_POST['password1']!=$col['password']){
  ...
  if($err==1){
    print" <FORM action='$PHP_SELF' method='POST'>
     ユーザ名  <b>".$_SESSION['org'].
    "</b>&nbsp;&nbsp;&nbsp;ユーザID  <b>".$_SESSION['login_id']."</b><br><br>
                現在のパスワード<br>
