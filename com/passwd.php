...
/****************************************/
 ...
 mysqli_query($con,"set names utf8");
  ...
  if((strlen($_POST['password1'])>0)&&(strlen($_POST['password2'])>0)) {
    //現在のパスワードが入力されたとき
    if(strlen($_SESSION['com_name'])>0){
	  //DBからパスワード抽出し比較
      $sql="SELECT passwd FROM com_tbl WHERE com_id =".$_SESSION['com_id'];
      ...
      if(!password_verify( $_POST['password1'], $col['passwd'])){
        ...
        $pw = password_hash($_POST['password2'],PASSWORD_DEFAULT);
  	    $sql= "UPDATE com_tbl SET passwd = \"$pw\" WHERE com_id=".$_SESSION['com_id'];
	    $rst = mysqli_query($con,$sql);
	    if($rst){
  ...
  if($err==1){
    print" <FORM action='$PHP_SELF' method='POST'>
     ユーザ名  <b>".$_SESSION['com_name'].
    "</b>&nbsp;&nbsp;&nbsp;ユーザID  <b>".$_SESSION['com_id']."</b><br><br>
                現在のパスワード<br>
